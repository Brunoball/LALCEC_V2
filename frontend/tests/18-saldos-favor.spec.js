...
test.describe('Saldos a favor de socios', () => {
  ...
  test('la UI muestra Saldos a favor y ofrece usarlo automáticamente dentro del modal de pago', async ({ page, request }) => {
    ...
    const printedReceipt = popup.locator('.gcuotas-comprobante');
    await expect(printedReceipt).toHaveCount(1);
    await expect(printedReceipt).toBeVisible();
    await expect(printedReceipt.locator('.gcuotas-talon-socio')).toContainText('Categoría / Monto abonado');
    await expect(printedReceipt.locator('.gcuotas-talon-socio')).toContainText('Saldo a favor aplicado');
    await expect(printedReceipt.locator('.gcuotas-talon-socio')).toContainText('Total de cuotas');
    await expect(printedReceipt.locator('.gcuotas-talon-cobrador')).toContainText('Categoría / Monto abonado');
    await expect(printedReceipt.locator('.gcuotas-talon-cobrador')).toContainText('Saldo a favor aplicado');
    await expect(printedReceipt.locator('.gcuotas-talon-cobrador')).toContainText('Total de cuotas');
    await popup.close();
  });
});
