...
final class TestingCleanup
{
    ...
    private static function realDataSnapshot(PDO $db): array
    {
        ...
        $queries = [
            ...
            // ultimo_uso es un heartbeat operativo: require_auth() lo actualiza cada minuto
            // cuando un usuario REAL navega por el sistema. Incluirlo en la huella hacía
            // fallar una corrida E2E contra producción aunque Playwright no hubiera tocado
            // ningún dato real. Se excluye SOLO ese campo volátil; siguen protegidos
            // idSesion, session_key, idUsuario, expira_en, ip, user_agent y activo.
            'sis_sesiones' => "SELECT s.idSesion, s.session_key, s.idUsuario, s.expira_en, s.ip, s.user_agent, s.activo FROM sis_sesiones s WHERE COALESCE(s.user_agent,'') NOT LIKE 'LALCEC-PLAYWRIGHT-E2E%' AND NOT EXISTS (SELECT 1 FROM sis_usuarios u WHERE u.idUsuario = s.idUsuario AND (u.usuario LIKE 'pw_e2e_%' OR COALESCE(u.email,'') LIKE '%@example.test')) ORDER BY s.idSesion",
